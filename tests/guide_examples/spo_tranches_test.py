...
def test_spo_tranches():
    ...
    strategy = inv.portfolio.strategy.SPOTranches(
        ...
        constraints=[
            ...
            MinWeightConstraint(limit=-0.03 / n_periods_held),
            MaxWeightConstraint(limit=0.03 / n_periods_held),
            LongCashConstraint(),
    ...
    assert round(backtest_result.annualized_return, 4) == 0.0355
    assert round(backtest_result.excess_risk_annualized, 4) == 0.0031
    assert round(backtest_result.information_ratio, 2) == 1.55
    assert round(backtest_result.annual_turnover, 2) == 6.29
    assert round(backtest_result.portfolio_hit_rate, 3) == 0.798
