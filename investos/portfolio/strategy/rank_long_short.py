...
class RankLongShort(BaseStrategy):
    """Investment strategy that builds trade list by going long assets with best metric_to_rank and short stocks with worst metric_to_rank.
    ...
    def generate_trade_list(self, holdings: pd.Series, t: dt.datetime) -> pd.Series:
        ...
        """
        weights_portfolio = self._get_trade_weights_for_t(holdings, t)
        dollars_trades = sum(holdings) * weights_portfolio * self.leverage_per_trade

        idx_t = self.metric_to_rank.index.get_loc(t)
        positions_saved = self.backtest_controller.results.dollars_holdings.shape[0]

        if positions_saved >= self.n_periods_held:
            # Use holdings_unwind, t_unwind, weights_portfolio_unwind, dollars_trades_unwind, dollars_trades_unwind_scaled
            t_unwind = self.metric_to_rank.index[idx_t - self.n_periods_held]
            holdings_unwind = self.backtest_controller.results.dollars_holdings.loc[
                t_unwind
            ]
            weights_portfolio_unwind = self._get_trade_weights_for_t(
                holdings_unwind, t_unwind
            )
            dollars_trades_unwind_pre = (
                sum(holdings_unwind)
                * weights_portfolio_unwind
                * self.leverage_per_trade
            )
            dollars_trades_unwind_scaled = (
                dollars_trades_unwind_pre
                * self._cum_returns_to_scale_unwind(t_unwind, t)
            )
    ...
    def _get_trade_weights_for_t(self, holdings: pd.Series, t: dt.datetime):
        ...
        prediction = get_value_at_t(self.metric_to_rank, t)
        ...
        weights_portfolio = pd.Series(0.0, index=prediction.index)
        weights_portfolio[short_trades] = (
            -1.0 * self.ratio_short * (self.percent_long / self.percent_short)
        )
        weights_portfolio[long_trades] = 1.0 * self.ratio_long

        weights_portfolio /= sum(abs(weights_portfolio))

        return weights_portfolio
