...
class FactorCovAdjuster:
    ...
    def __init__(
        ...
        self.datetime_index = factor_return_matrix.index[
            [
                idx
                for idx in range(
    ...
    def convert_cov_matrices(self, cov_matrices: list) -> pd.DataFrame:
        ...
        return pd.DataFrame(
            cov_matrices.reshape((-1, cov_matrices.shape[-1])),
            index=multi_index,
            columns=self.factor_index,
        )
    ...
    def newey_west_adjust(
        ...
        eigen_vals[eigen_vals <= 0] = (
            1e-14  # adjust diagonal matrix to be positive definite
        )
    ...
    def apply_newey_west_adjustment(
        self,
        max_lags: int = 1,
        ...
        if not self.window:
            ...
            for idx, val in enumerate(
                self.factor_cov_matrix.index.get_level_values(0).unique()
            ):
                cov_matrices.append(
                    self.newey_west_adjust(
                        factor_cov_matrix=self.factor_cov_matrix.loc[val].T.to_numpy(),
                        factor_return_matrix=self.factor_return_matrix.iloc[
                            self.factor_return_matrix.shape[0]
                            - idx * self.recalc_freq
                            - self.window : self.factor_return_matrix.shape[0]
                            - idx * self.recalc_freq,
                            :,
                        ].T.to_numpy(),
                        half_life=half_life,
    ...
    def eigenfactor_risk_adjust(
        ...
        for _ in range(num_sim):  # number of MC simulation
            sim_eigen_factor_matrix = np.array(
                [np.random.normal(0, var**0.5, window) for var in eigen_vals_0]
            )
            sim_factor_return_matrix = eigen_vecs_0.dot(sim_eigen_factor_matrix)
            sim_factor_return_cov = sim_factor_return_matrix.dot(
                sim_factor_return_matrix.T
            ) / (window - 1)
            sim_eigen_vals, sim_eigen_vecs = np.linalg.eigh(sim_factor_return_cov)
            sim_eigen_vals[sim_eigen_vals <= 0] = (
                1e-14  # adjust diagonal matrix to be positive definite
            )
            sim_real_eigen_vals = sim_eigen_vecs.T.dot(factor_cov_matrix).dot(
                sim_eigen_vecs
            )
            sim_risk_deviation += np.diag(sim_real_eigen_vals) / sim_eigen_vals
        sim_risk_deviation[sim_risk_deviation <= 0] = (
            1e-14  # adjust diagonal matrix to be positive definite
        )
        ...
        factor_cov_matrix = eigen_vecs_0.dot(np.diag(eigen_vals_0_tilde)).dot(
            eigen_vecs_0.T
        )
        return factor_cov_matrix

    def apply_eigenfactor_risk_adjustment(
        self,
        max_lags: int = 1,
        multiplier: float = 1.2,
        half_life: int | None = 480,
        ...
        if not self.window:
            cov_matrices.append(
                self.eigenfactor_risk_adjust(
                    self.factor_cov_matrix, coef, window, num_of_sim
                )
            )
        else:
            for _, val in enumerate(
                ...
                cov_matrices.append(
                    self.eigenfactor_risk_adjust(
                        self.factor_cov_matrix.loc[val].T.to_numpy(),
                        coef,
                        window,
                        num_of_sim,
                    )
    ...
    def apply_volatility_regime_adjustment(self, half_life: int | None = 480):
        ...
        if not self.window:
            cov_matrices.append(
                self.volatility_regime_adjust(
                    self.factor_cov_matrix,
                    self.factor_return_matrix.T.to_numpy(),
                    half_life,
                )
            )
        else:
            for idx, val in enumerate(
                ...
                cov_matrices.append(
                    self.volatility_regime_adjust(
                        self.factor_cov_matrix.loc[val].T.to_numpy(),
                        self.factor_return_matrix.iloc[
                            self.factor_return_matrix.shape[0]
                            - idx * self.recalc_freq
                            - self.window : self.factor_return_matrix.shape[0]
                            - idx * self.recalc_freq,
...
class AssetDigonalVarAdjuster:
    def __init__(
        ...
        if window:
            ...
            self.window = 0
        self.recalc_freq = recalc_freq
        self._df_idio_returns = df_idio_returns.set_index(
            ["datetime", "id"]
        ).sort_values(by="datetime")
        self.datetime_index = self._df_idio_returns.index.get_level_values(0).unique()[
            [
                idx
                for idx in range(
                    len(self._df_idio_returns.index.get_level_values(0).unique()) - 1,
                    self.window,
                    -self.recalc_freq,
                )
            ]
    ...
    def convert_cov_matrices(self, cov_matrices: list) -> pd.DataFrame:
        ...
        for idx in range(len(cov_matrices)):
            ...
            cov_matrices[idx] = (
                cov_matrices[idx].reset_index().set_index(["datetime", "id"])
            )
    ...
    def idio_var_emwa(self, df_idio_returns, half_life: int = 360):
        ...
        df_idio_returns_cp.loc[:, "weight"] = np.exp(
            -decay_factor
            * (
                df_idio_returns_cp["datetime"].max() - df_idio_returns_cp["datetime"]
            ).dt.days
        )

        # Calculate weighted returns squared
        df_idio_returns_cp.loc[:, "weighted_squared_returns"] = (
            df_idio_returns_cp["weight"]
            * df_idio_returns_cp["factor_return_1d_error"] ** 2
        )

        # Calculate the EWMA variance for each 'id'
        ewma_variance = df_idio_returns_cp.groupby("id").apply(
            lambda x: x["weighted_squared_returns"].sum() / x["weight"].sum()
        )

        return pd.DataFrame(
            ewma_variance, index=ewma_variance.index, columns=["idio_variance"]
        )  # this is variance
    ...
    def est_ewma_idiosyncratic_variance(self, half_life: int = 360):
        ...
        if not self.window:
            cov_matrices.append(
                self.idio_var_emwa(
                    df_idio_returns=self._df_idio_returns, half_life=half_life
                )
            )
        else:
            for date in self.datetime_index:
                cov_matrices.append(
                    self.idio_var_emwa(
                        df_idio_returns=self._df_idio_returns.loc[
                            date - pd.Timedelta(days=self.window) : date, :
                        ],
                        half_life=half_life,
                    )
                )
