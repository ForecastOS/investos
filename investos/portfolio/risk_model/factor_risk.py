...
class FactorRisk(BaseRisk):
    ...
    def _pull_fos_risk_factor_dfs(self):
        ...
        if not fos.api_key:
            raise ValueError(
                "Feature hub connection failed. Please check the API key is valid."
            )
        ...
        for factor in self._fos_risk_factor_uuids:
            ...
            if "datetime" not in temp_df.columns:
                ...
                self._df_loadings = self._df_loadings.merge(
                    temp_df, how="left", on="id"
                )
        ...
        self._df_loadings = self._df_loadings.replace(
    ...
    def _generate_risk_models(self, timestamp="datetime") -> None:
        ...
        self._fos_industry_factors = [
            col
            for col in self._df_loadings.columns
            if col
            not in ["datetime", "id"]
        ...
        for date in sorted(self._df_loadings[timestamp].unique()):
            ...
            y, X = (
                ...
                df_current.loc[
                    :, (self._fos_risk_factor_uuids + self._fos_industry_factors)
                ],
            )
            X.loc[:, "const"] = (
                1  # need the intercept column, will convert this to country factor later
            )
            model = sm.OLS(y, X)
            results = model.fit()
            ...
            self._factor_returns[date] = {
                ...
                "intercept": results.params[
                    "const"
                ],  # the intercept needs the X col of all 1
    ...
    def _generate_idiosyncratic_returns(self):
        ...
        self._df_idio = (
            self._df_idio.drop(columns=self._fos_risk_factor_uuids)
            .drop(columns=self._fos_industry_factors)
            .drop(columns=self.cols_with_return)
    ...
    def _generate_factor_returns(self):
        ...
        list_to_insert = [
            [
                ...
                *self._factor_returns[k]["t-values"][:-1],
                *self._factor_returns[k]["coefficients"][:-1],
            ]
            for k in self._factor_returns
        ]  # iterative through the factor returns dict

        # split the columns into return, t-value and r2
        self.cols_with_return = [
            "returns_" + col
            for col in (self._fos_risk_factor_uuids + self._fos_industry_factors)
        ]
        self.cols_with_t_values = [
            "t_values_" + col
            for col in (self._fos_risk_factor_uuids + self._fos_industry_factors)
        ]
        self._df_factor_summary = pd.DataFrame(
            list_to_insert,
            columns=[
                "datetime",
                "r2",
    ...
    def _apply_risk_model_adjustments(self):
        ...
        if (
            ...
            factorcovadjuster = FactorCovAdjuster(
                self._df_factor_returns,
                window=self._risk_model_window,
                recalc_freq=self._recalc_freq,
            )
            self._df_cov_raw = factorcovadjuster.est_factor_cov_matrix_raw(
                self._df_factor_returns,
